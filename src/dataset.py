...
__cached_df = {}
__dataset_suffix = {"train": "_train", "valid": "_valid", "test": "_test"}
__discarded_suffix = {
    False: "",
    True: "_discarded",
}
...
def read_dataset(
    stage: Literal["original", "preprocessed", "regression", "classification"],
    ...
    df_x = __load_df(
        f"../dataset/{stage_id}-{stage}/X{__dataset_suffix[dataset]}{__discarded_suffix[discarded]}.csv",
        use_cache=use_cache,
    ).drop(columns=["id"], errors="ignore")
    if dataset == "train":
        df_y = __load_df(
            f"../dataset/{stage_id}-{stage}/y{__discarded_suffix[discarded]}.csv", use_cache=use_cache
        ).drop(columns=["id"], errors="ignore")
    ...
    if stage == "classification":
        base_dir = (
            "../dataset/2-regression/y"
            if cheat and dataset == "train"
            else f"../2-torque_target_probabilistic_regression/predictions{__dataset_suffix[dataset]}"
        )
        df_x["trq_margin"] = __load_df(f"{base_dir}{__discarded_suffix[discarded]}.csv", use_cache=use_cache)[
            "trq_margin"
        ]
...
def split_dataset(
    ...
    device="cpu",
    group_testset=False,
) -> tuple[tuple[Tensor, Tensor], tuple[Tensor, Tensor], tuple[Tensor, Tensor], dict[str, float]]:
    ...
    if group_by is None:
        ...
        if group_testset:
            testset = {
                group: (
                    test_x[tensor((group_col == group)[len(train_x) + len(valid_x) :].values)],
                    test_y[tensor((group_col == group)[len(train_x) + len(valid_x) :].values)],
                )
                for group in groups
            }
        else:
    ...
    return trainset, validset, testset, normalizations
